...
public class PendingQueueTests {
    ...
    void test_offer_multi_poll(int initialLoad, int maxFirstChunkSize, int maxSecondChunkSize, int maxWriters) throws InterruptedException {
        // given
        long start = System.currentTimeMillis();
        log.info(String.format("Running %d writers with chunks of %d and %d elements, starting from %d elements", maxWriters, maxFirstChunkSize, maxSecondChunkSize, initialLoad));
        List<Task> writers = new LinkedList<>();
        final PendingQueue<Integer> queue = new PendingQueue<>();
        PendingQueue<Integer>.OrderIterator orderIterator = (PendingQueue<Integer>.OrderIterator) queue.orderIterator();
        ...
        IntStream.rangeClosed(1, maxWriters)
                .forEach(i ->
                        writers.add(new Task(orderIterator, doneSignal, readerEndsUp, errors))
                );

        // when
        IntStream.rangeClosed(1, initialLoad)
                .forEach(queue::add
                );
        VirtualThreadExecutor executor = new VirtualThreadExecutor("writer");
        writers.forEach(executor::execute);
        Thread reader = new Thread(() -> {
            IntStream.rangeClosed(initialLoad+1, maxFirstChunkSize)
                    .forEach(queue::offer
                    );
            try {
    ...
    void test_order_iterator_with_filter() throws InterruptedException {
        ...
        PendingQueue<Integer>.OrderIterator orderIterator = (PendingQueue<Integer>.OrderIterator) queue.orderIterator(filter);
        ...
        // then
        Assertions.assertArrayEquals(validElements.toArray(new Integer[0]), gottenElements.toArray(new Integer[0]));
    ...
    void test_purge_with_order_iterator_02() {
        ...
        // when
        PendingQueue<Integer>.OrderIterator orderIterator2 = (PendingQueue<Integer>.OrderIterator) queue.orderIterator();
